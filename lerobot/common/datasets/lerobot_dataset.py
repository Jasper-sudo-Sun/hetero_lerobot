...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # def filter_max_task_index(dataset):
        #     # 获取所有 task_index 的值
        #     task_indices = [int(x) for x in dataset["task_index"]]

        #     # 统计频率
        #     counts = Counter(task_indices)

        #     # 找出数量最多的那个 task_index
        #     max_index = counts.most_common(1)[0][0]

        #     # 过滤出只包含该 task_index 的样本
        #     filtered_dataset = dataset.filter(lambda x: int(x["task_index"]) == max_index)

        #     return filtered_dataset
        
        # self.hf_dataset = filter_max_task_index(self.hf_dataset)
        # print("Now task index is:", int(self.hf_dataset.select([0])["task_index"][0]))
        # self.start_episodes_index = int(self.hf_dataset.select([0])["episode_index"][0])
        # self.episodes = sorted(set(int(x) for x in self.hf_dataset["episode_index"]))
        # self.meta.episodes = {
        #     k: v for k, v in self.meta.episodes.items() if int(k) in self.episodes 
        # }
        # #!+++++++++++++++++++++++++++++++++++++qianpusun+++++++++++++++++++++++++++++++++++
    ...
    def _get_query_indices(self, idx: int, ep_idx: int) -> tuple[dict[str, list[int | bool]]]:
        # ep_start = self.episode_data_index["from"][ep_idx - self.start_episodes_index]
        # ep_end = self.episode_data_index["to"][ep_idx - self.start_episodes_index]
