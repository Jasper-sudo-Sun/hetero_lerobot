...
class ACTPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        hetero_configs: dict[str, ACTConfig] | None = None,
        dataset_stats: dict[str, dict[str, Tensor]] | None = None,
        hetero_dataset_stats: dict[str, dict[str, Tensor]] | None = None,
    ):
        """
        Args:
            config: Policy configuration class instance or None, in which case the default instantiation of
                    the configuration class is used.
            repo_id: The repository ID for the policy.
            dataset_stats: Dataset statistics to be used for normalization. If not passed here, it is expected
                that they will be passed with a call to `load_state_dict` before the policy is used.
        """
        super().__init__(config)

        if hetero_configs is not None:
            for hetero_repo_id in hetero_configs.keys():
                hetero_configs[hetero_repo_id].validate_features()
        else:
        ...
        if hetero_dataset_stats is not None:
            for key in hetero_dataset_stats.keys():
                ...
                self.normalize_targets[key] = Normalize(
                    hetero_configs[key].output_features, hetero_configs[key].normalization_mapping, hetero_dataset_stats[key]
                ).cuda()
                self.unnormalize_outputs[key] = Unnormalize(
                    hetero_configs[key].output_features, hetero_configs[key].normalization_mapping, hetero_dataset_stats[key]
                ).cuda()
    ...
    @torch.no_grad
    def forward_action(self, batch: dict[str, Tensor]) -> Tensor:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        normalize_inputs_instances = [self.normalize_inputs[repo_id] for repo_id in batch["repo_id"]]
        for key in self.config.input_features.keys():
            batch_dict = [normalize_inputs_instance({key: batch[key][i].unsqueeze(0)}) for i, normalize_inputs_instance in enumerate(normalize_inputs_instances)]
            batch_data = torch.cat([d[key] for d in batch_dict], dim=0)
            batch[key] = batch_data
        ...
        normalize_targets_instances = [self.normalize_targets[repo_id] for repo_id in batch["repo_id"]]
        for key in self.config.output_features.keys():
            batch_dict = [normalize_targets_instance({key: batch[key][i].unsqueeze(0)}) for i, normalize_targets_instance in enumerate(normalize_targets_instances)]
            batch_data = torch.cat([d[key] for d in batch_dict], dim=0)
            batch[key] = batch_data

        actions_hat, (mu_hat, log_sigma_x2_hat) = self.model(batch)
        
        non_zero_mask = ~(batch['action'] == 0)
        l1_loss = (
            F.l1_loss(batch["action"][non_zero_mask], actions_hat[non_zero_mask], reduction="none")
        ).mean()
